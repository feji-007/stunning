...
router.get('/me', adminRequired, async (req, res) => {
  const row = await db.get('SELECT * FROM admins WHERE id = ?', req.admin.id);
...
router.put('/password', adminRequired, async (req, res) => {
  ...
  await db.run('UPDATE admins SET password_hash = ?, updated_at = ? WHERE id = ?', hash, Date.now(), req.admin.id);
  res.json({ ok: true });
});
