...
router.put('/profile', authRequired, (req, res) => {
  ...
  if (typeof avatar === 'string') {
    // 头像可以是 data URL 或图片 URL，限制长度避免过大
...
// 查询积分
router.get('/points', authRequired, (req, res) => {
  const row = db.prepare('SELECT points FROM users WHERE id = ?').get(req.user.id);
  if (!row) return res.status(404).json({ error: '用户不存在' });
...
router.post('/points', authRequired, (req, res) => {
  ...
  db.prepare(`
    UPDATE users SET points = MAX(0, points + ?), updated_at = strftime('%s','now') * 1000 WHERE id = ?
  `).run(delta, req.user.id);
  const row = db.prepare('SELECT points FROM users WHERE id = ?').get(req.user.id);
  res.json({ points: row.points });
});
